...
class Pong:
    ...
    # def set_ai_move(self, move, ai_id):
    #     self.ai_move[ai_id] = move
    #
    # def get_ai_move(self, ai_id):
    #     current_move = copy.copy(self.ai_move[ai_id])
    #     self.ai_move[ai_id] = None
    #     return current_move
    ...
    def play(self):
        ...
        while carryOn:
            ...
            if self.ai_player_count == 1:
                print("In ai player step")
                try:
                    ai_move = self.ai_1_queue.get_nowait()
                    print(ai_move)
                    if ai_move == "UP":
                        paddleA.moveUp(5)
                    if ai_move == "DOWN":
                    ...
                    print("No ai moves in queue")
                if keys[pygame.K_UP]:
                    paddleB.moveUp(5)
                if keys[pygame.K_DOWN]:
                    paddleB.moveDown(5)
            # if self.ai_player_count == 2:
            #     if keys[pygame.K_UP]:
            #         paddleB.moveUp(5)
            #     if keys[pygame.K_DOWN]:
            #         paddleB.moveDown(5)
            #     self.ai_move(paddleB)
            #     self.ai_move(paddleA)
